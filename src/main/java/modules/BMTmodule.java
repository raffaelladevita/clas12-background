 ...
 * @author devita
 */
public class BMTmodule extends Module {

    private static final int NREGIONS = 3;
    private static final int NSECTORS = 3;
    private static final int NLAYERS = 6;
    private static final int NSL = 6;
    private static final int[] NSTRIPS = {160, 430, 510, 370, 750, 540};
    ...
    public DataGroup occupancy_region() {
        ...
        for (int ir = 0; ir < NREGIONS; ir++) {
            ...
            H1F hi_wire = histo1D("hi_wire_" + name, " ", "Wire", "Occupancy[%] ", NSTRIPS[4], 0.5, NSTRIPS[4] + 0.5, 0);
    ...
    public void fillHistos(Event event) {
        List<Hit> allhits = event.getHits(DetectorType.BMT);
        if (allhits!=null) {
...
//            System.out.println(hit.getTrue().getEdep() + " " + hit.getTrue().getTime()+ " " + hit.getTDC());
            ...
            double phi = hit.getTrue().getPosition().toVector3D().phi();
            phi = (phi+2.475*Math.PI)%(Math.PI*110/180);
            int cstrip = (int) ((z-rho*Math.toRadians(20))/0.4);
            int zstrip = (int) (phi*rho/0.487);
            if(hit.getLayer()==1 || hit.getLayer()==4 || hit.getLayer()==6) {
                group.getH2F("hi_occ_sector" + hit.getSector()).fill(hit.getComponent(), hit.getLayer()*2-1,this.readoutWeight(hit.getLayer()));
                group.getH2F("hi_occ_sector" + hit.getSector()).fill(cstrip, hit.getLayer()*2,this.readoutWeight(hit.getLayer()));
            }
            else {
                group.getH2F("hi_occ_sector" + hit.getSector()).fill(zstrip, hit.getLayer()*2-1,this.readoutWeight(hit.getLayer()));
                group.getH2F("hi_occ_sector" + hit.getSector()).fill(hit.getComponent(), hit.getLayer()*2,this.readoutWeight(hit.getLayer()));
            }
        }
    }
    ...
    public void fillOccupancy_region(DataGroup group, List<Hit> hits) {
        for (Hit hit : hits) {
            int region = (hit.getLayer() - 1) / (NLAYERS/NREGIONS) + 1;
            int sl = (hit.getLayer() - 1) / (NLAYERS/NSL) + 1;
            group.getH1F("hi_occ_region" + region).fill(hit.getSector(),this.readoutWeight(hit.getLayer()));
            group.getH1F("hi_wire_region" + region).fill(hit.getComponent(),this.readoutWeight(hit.getLayer())*NSTRIPS[hit.getLayer()-1]/NSECTORS);
            group.getH1F("hi_layer").fill(hit.getLayer(),this.readoutWeight(hit.getLayer())*NLAYERS/NREGIONS/NSECTORS);
            group.getH1F("hi_sl").fill(sl,this.readoutWeight(hit.getLayer())/NSECTORS*2);
        }
    }
...
//            System.out.println(weight);
            ...
            if(this.pidToName(Math.abs(Math.abs(t.getPid())))!=null) {
                if(t.getPid()==22) {
                    ...
                    if(t.getEnergy()>0.01)
                        group.getH1F("hi_bg_region" + region + "_" + "gamma0").fill(value);                        
                }
                else
                    group.getH1F("hi_bg_region" + region + "_" + this.pidToName(Math.abs(t.getPid()))).fill(value);
            }
